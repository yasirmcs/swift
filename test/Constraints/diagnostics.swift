...
func f20371273() {
  let x: [Int] = [1, 2, 3, 4]
  let y: UInt = 4
  x.where { $0 == y }  // expected-error {{binary operator '==' cannot be applied to operands of type 'Int' and 'UInt'}}
...
var numbers = [1, 2, 3]
zip(numbers, numbers).where { $0.2 > 1 }  // expected-error {{value of tuple type '(Int, Int)' has no member '2'}}
...
func f(_ arguments: [String]) -> [ArraySlice<String>] {
  return arguments.split(maxSplits: 1, omittingEmptySubsequences: false, separatedWhere: { $0 == "--" })
}
